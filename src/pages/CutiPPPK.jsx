...
const CutiPPPK = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (response.data.jenisPegawai === 'PPPK') {
          setIsPPPK(true);
        } else {
          setNotification("Anda bukan pegawai jenis PPPK dan tidak boleh mengakses halaman ini.");
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isRequiredFieldsFilled && isPPPK) {
      try {
        const response = await axios.post("http://localhost:8000/api/cuti/pppk/", {
          nama_pegawai: nama,
          nip_baru: nip,
      ...
      setNotification("Anda bukan pegawai jenis PPPK dan tidak dapat mengajukan cuti di halaman ini.");
    } else {
      setIsFormComplete(false);
  ...
  return (
    <div style={{ backgroundColor: "#1E1E2D", minHeight: "100vh" }}>
      ...
      <div className="d-flex">
        <Sidebar />
        <div className="p-4 text-white w-100">
          <h2 className="mb-4">Formulir Pengajuan Cuti PPPK</h2>{notification && (
            <div
          ...
          <form onSubmit={handleSubmit} className="cuti-form p-4 rounded shadow">
            ...
            <div className="d-flex justify-content-end">
              <button type="submit" className="btn btn-success" disabled={!isPPPK}>
                Ajukan Cuti
              </button>
            </div>
          </form>
